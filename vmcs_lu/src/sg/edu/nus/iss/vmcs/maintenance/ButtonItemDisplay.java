...
/**
 * This boundary object is to display stock information when a button is
 * pressed.
 *
...
public class ButtonItemDisplay extends Panel {
	 ...
	 * @param sitem
	 *            the array of StoreObject.
	 * @param length
	 *            the length of StoreObject array.
	 */
	public ButtonItemDisplay(String title, StoreItem sitem[], int length) {
		...
		for (i = 0; i < len; i++) {
			...
			items[i] = new ButtonItem(ob.getName(), ButtonItem.DEFAULT_LEN, ButtonItem.GRID);
			this.add(items[i]);
		}
	}
