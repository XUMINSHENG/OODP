...
public class DrinkSelectionBox extends Panel {
	...
	public DrinkSelectionBox(TransactionController txCtrl) {
		...
		for (int i = 0; i < drinkStoreItems.length; i++) {
			...
			add(drinkSelectionItems[i], new GridBagConstraints(0, i, 1, 1, 1.0, 0.0, GridBagConstraints.CENTER,
					GridBagConstraints.HORIZONTAL, new Insets(5, 0, 0, 0), 10, 0));
		}
	}

	/**
	 * This method updates the stock status, name and price of a drink brand
	 ...
	 * 
	 * @param active
	 *            TRUE to activate, FALSE to deactivate the drink selection
	 *            buttons.
	 */
	...
	/**
	 * This method activates or deactivates the drink selection buttons on the
	 * Customer Panel.
	 * 
	 * @param index
	 *            the index of the drink brand.
	 * @param active
	 *            TRUE to activate, FALSE to deactivate the drink selection
	 *            buttons.
	 */
	public void setState(int index, boolean active) {
		if (drinkSelectionItems == null || drinkSelectionItems.length == 0 || index < -1
				|| index >= drinkSelectionItems.length)
			return;
	...
	/**
	 * This method activates or deactivates the drink selection buttons on the
	 * Customer Panel. This method also displays OUT OF STOCK messages where
	 * appropriate.
	 * 
	 * @param index
	 *            the index of the drink brand.
